...
- **`gh` config UID mapping**: The gh-config disk device MUST use `shift=true` for kernel idmapped mounts. In unprivileged LXD containers, host UID 1000 maps to `nobody` (65534) inside the container. Since gh config files have `0600` permissions, they are unreadable without UID remapping. `shift=true` transparently remaps host UID 1000 → container UID 1000 (requires kernel idmapped mounts support, present on kernel 6.x+ with LXD 5.x+).
...
- Q: Why does `gh auth status` fail with "permission denied" inside the container? → A: LXD's unprivileged container UID remapping maps host UID 1000 to `nobody` (65534) inside the container. Since gh config files have `0600` permissions, the container's ubuntu user can't read them. Fix: `shift=true` on the gh-config disk device enables kernel idmapped mounts, transparently remapping host UID 1000 → container UID 1000.
